<template>
  <div class="con">
    <mark>泡泡外刊</mark>
    <svg
      class="icon"
      viewBox="0 0 1036 1024"
      width="24"
      height="24"
      @click="$router.push('/')"
    >
      <path
        d="M1014.628571 504L543.885714 33.6a36.491429 36.491429 0 0 0-51.657143 0L21.485714 504c-13.714286 13.714286-21.485714 32.342857-21.485714 51.771429 0 40.342857 32.8 73.142857 73.142857 73.142857h49.6V964.571429c0 20.228571 16.342857 36.571429 36.571429 36.571428H444.914286V745.142857h128v256h303.885714c20.228571 0 36.571429-16.342857 36.571429-36.571428V628.914286h49.6c19.428571 0 38.057143-7.657143 51.771428-21.485715 28.457143-28.571429 28.457143-74.857143-0.114286-103.428571z"
      ></path>
    </svg>

    <!-- <svg class="icon" viewBox="0 0 1024 1024" width="24" height="24">
      <path
        d="M938.666667 426.666667L682.666667 170.666667H170.666667c-46.933333 0-85.333333 38.4-85.333334 85.333333v512.426667C85.333333 815.36 123.733333 853.333333 170.666667 853.333333l682.666666-0.426666c46.933333 0 85.333333-37.973333 85.333334-84.906667V426.666667z m-298.666667-192l234.666667 234.666666H640V234.666667z"
      ></path>
    </svg> -->

    <svg
      class="icon"
      viewBox="0 0 1024 1024"
      width="24"
      height="24"
      @click="$router.push('/about')"
    >
      <path
        d="M512 32C247.04 32 32 247.04 32 512s215.04 480 480 480 480-215.04 480-480S776.96 32 512 32z m58.56 725.76c0 25.92-21.12 47.04-47.04 47.04h-23.52c-25.92 0-47.04-21.12-47.04-47.04V476.96c0-25.92 21.12-47.04 47.04-47.04h23.52c25.92 0 47.04 21.12 47.04 47.04v280.8zM512 359.84c-32.16 0-58.56-26.4-58.56-58.56 0-32.16 26.4-58.56 58.56-58.56s58.56 26.4 58.56 58.56c0 32.16-26.4 58.56-58.56 58.56z"
      ></path>
    </svg>

    <a href="https://github.com/wushanglang/ppenglish" target="_blank">
      <svg class="icon" viewBox="0 0 1024 1024" width="24" height="24">
        <path
          d="M511.6 76.3C264.3 76.2 64 276.4 64 523.5 64 718.9 189.3 885 363.8 946c23.5 5.9 19.9-10.8 19.9-22.2v-77.5c-135.7 15.9-141.2-73.9-150.3-88.9C215 726 171.5 718 184.5 703c30.9-15.9 62.4 4 98.9 57.9 26.4 39.1 77.9 32.5 104 26 5.7-23.5 17.9-44.5 34.7-60.8-140.6-25.2-199.2-111-199.2-213 0-49.5 16.3-95 48.3-131.7-20.4-60.5 1.9-112.3 4.9-120 58.1-5.2 118.5 41.6 123.2 45.3 33-8.9 70.7-13.6 112.9-13.6 42.4 0 80.2 4.9 113.5 13.9 11.3-8.6 67.3-48.8 121.3-43.9 2.9 7.7 24.7 58.3 5.5 118 32.4 36.8 48.9 82.7 48.9 132.3 0 102.2-59 188.1-200 212.9 23.5 23.2 38.1 55.4 38.1 91v112.5c0.8 9 0 17.9 15 17.9 177.1-59.7 304.6-227 304.6-424.1 0-247.2-200.4-447.3-447.5-447.3z"
        ></path>
      </svg>
    </a>
  </div>
</template>

<script>
export default {};
</script>

<style scoped>
.con {
  width: 100%;
  margin: 0 auto;
  display: flex;
  gap: 1rem;
  align-content: center;
}
.icon {
  cursor: pointer;
}
.icon:hover {
  fill: #444;
}

mark {
  user-select: none;
  font-weight: bold;
  font-size: 24px;
  line-height: 1.1;
  padding: 0;
  background: linear-gradient(to right, #222, #333, #444, #555, #666);
  -webkit-background-clip: text;
  background-clip: text;
  color: transparent;
}
</style>